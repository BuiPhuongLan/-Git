...
class Emitter(filename:String) {
  ...
  { 
	...
	*/
  	...
  	def emitNOT(in:Type,frame:Frame ) =
	{
      ...
      result.append(emitPUSHICONST("true",frame));
      result.append(emitGOTO(label2,frame));
      result.append(emitLABEL(label1,frame));
      result.append(emitPUSHICONST("false",frame));
	...
	*	@param in the type of the operands.
	*/	
	def emitAROP(lexeme:String, in:Type,frame:Frame) = {
 ...
 //    else
	...
	*/	
    ...
    def emitFCMPL(op:String,frame:Frame) = {
    	val result = new StringBuffer();
    	...
    	op match {
    		case ">" => result.append(jvm.emitIFLE(labelF))
    		case ">=" => result.append(jvm.emitIFLT(labelF))
    		case "<" => result.append(jvm.emitIFGE(labelF))
    		case "<=" => result.append(jvm.emitIFGT(labelF))
    	}
